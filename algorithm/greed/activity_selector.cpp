...
假设我们存在这样一个活动集合S=a1,a2,a3,a4,...,an,其中每一个活动ai都有一个开始时间si和结束时间fi保证(0<=si<fi),活动ai进行时，那么它占用的时间为[si,fi).现在这些活动占用一个共同的资源，就是这些活动会在某一时间段里面进行安排，如果两个活动ai和aj的占用时间[si,fi),[sj,fj)不重叠，那么就说明这两个活动是兼容的，也就是说当si<=fj或者sj<=fi,那么活动ai,aj是兼容的。(copy
from internal) 现在要求尽可能安排多的活动,就是在某个时间段里面,兼容活动越多越好
已知:
...
// 递归版本
void RecrusiveActivitySelector(int activity_len, int activity_num, int start[],
                               int end[], vector<int> *selected_activity) {
  selected_activity->push_back(activity_num);
  int i;
  for (i = activity_num + 1; i < activity_len && start[i] < end[activity_num];
       i++)
    ;
  if (i >= activity_len)
    return;
  activity_num = i;
  RecrusiveActivitySelector(activity_len, activity_num, start, end,
                            selected_activity);
}

// 迭代版本
void IterateActivitySelector(int activity_len, int activity_num, int start[],
                             int end[], vector<int> *selected_activity) {
...
int main() {
  ...
  for (int i = 0; i < count; i++) {
    cout << index[selected_activity.at(i)] << ":"
         << start[selected_activity.at(i)] << "->"
